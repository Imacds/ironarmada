...
namespace BeardedManStudios.Network
{
	public class CrossPlatformUDP : NetWorker
	{
...
#endif
					Me = new NetworkingPlayer(ServerPlayerCounter++, "127.0.0.1", new IPEndPoint(IPAddress.Parse("127.0.0.1"), Port), "SERVER");
		...
		{
			Me = new NetworkingPlayer(Uniqueidentifier, "127.0.0.1", new IPEndPoint(IPAddress.Parse("127.0.0.1"), Port), string.Empty);
				...
				lock (clientSocketMutex)
				{
					foreach (BMSByte packet in packets)
					{
						...
						foreach (KeyValuePair<string, NetworkingPlayer> kv in clientSockets)
						{
							try
							{
								Send(sendData, packet.Size, kv.Value.SocketEndpoint);
							}
							catch
							{
								Disconnect(kv.Value);
						...
						foreach (KeyValuePair<string, NetworkingPlayer> kv in clientSockets)
						{
							if (stream != null)
							{
								...
								if (stream.Receivers == NetworkReceivers.OthersProximity || stream.Receivers == NetworkReceivers.AllProximity)
								{
									...
									else if (!ReferenceEquals(stream.Sender.PlayerObject, null) && !ReferenceEquals(kv.Value.PlayerObject, null))
								...
								if ((stream.Receivers == NetworkReceivers.Owner || stream.Receivers == NetworkReceivers.ServerAndOwner) && !ReferenceEquals(stream.NetworkedBehavior, null) && kv.Value.NetworkId != stream.NetworkedBehavior.OwnerId)
								{
									if (reliable) RemoveReliable(updateidentifier, kv.Value);
									continue;
								}
							...
							{
								disconnectedPlayers.Add(kv.Value);
							}
						}
					}
		...
		private void PacketReceived(string endpoint, BMSByte bytes)
		{
			...
			if (header != null && header.reliable && clientSockets.ContainsKey(endpoint))
			{
				if (reliableMultiPartPending.ContainsKey(sender))
				{
					if (reliableMultiPartPending[sender].ContainsKey(header.updateId))
					{
						if (reliableMultiPartPending[sender][header.updateId].ContainsKey(header.packetGroupId) && reliableMultiPartPending[sender][header.updateId][header.packetGroupId].Value[header.packetOrderId] != null)
						{
			...
			else
			{
				...
				{
					// New player
					if (!clientSockets.ContainsKey(endpoint) && ObjectMapper.Compare<string>(readStream, "connect"))
					{
						lock (removalMutex)
						{
					...
					foreach (KeyValuePair<string, NetworkingPlayer> player in clientSockets)
					{
						...
						else
						{
							if ((DateTime.Now - player.Value.LastPing).TotalSeconds > player.Value.InactiveTimeoutSeconds)
								timeoutDisconnects.Add(player.Value);
						}
					}
